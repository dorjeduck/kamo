...
fn format_float[dtype: DType](f: Scalar[dtype], digits: Int = 2) -> String:
    ...
    var ff = String(Int(round(abs(f * 10**digits))))
    var l = len(ff)
...
](x1: SIMD[dtype, width], x2: SIMD[dtype, width]) -> SIMD[dtype, width]:
    return x1 + x2
...
struct MoNum[dtype: DType, simd_width: Int]:
    ...
    ) raises -> MoVector[dtype, simd_width]:
        var str_list = s.split(sep, maxsplit)

        var res = MoVector[dtype, simd_width](len(str_list))
    ...
    @staticmethod
    fn inplace_copy(
        target: MoVector[dtype, simd_width], source: MoVector[dtype, simd_width]
    ):
        memcpy(target._vec_ptr, source._vec_ptr, source.size)

    @staticmethod
    fn inplace_copy(
        target: MoMatrix[dtype, simd_width], source: MoMatrix[dtype, simd_width]
    ):
        memcpy(target._mat_ptr, source._mat_ptr, source.cols * source.rows)

    @staticmethod
    fn minmax(
        mv: MoVector[dtype, simd_width]
    ) -> (Scalar[dtype], Scalar[dtype]):
        return Self._minmax(mv._vec_ptr, mv.size)

    @staticmethod
    fn minmax(
        mm: MoMatrix[dtype, simd_width]
    ) -> (Scalar[dtype], Scalar[dtype]):
    ...
    @staticmethod
    fn sum(
        mm: MoMatrix[dtype, simd_width], axis: Int
    ...
    @staticmethod
    fn pow(
        mv: MoVector[dtype, simd_width], p: Scalar[dtype] = 2
    ) -> MoVector[dtype, simd_width]:
        return MoNum[dtype, simd_width]._elemwise_func_pow(mv, p)

    @staticmethod
    fn pow(
        mm: MoMatrix[dtype, simd_width], p: Scalar[dtype] = 2
    ) -> MoMatrix[dtype, simd_width]:
        return MoNum[dtype, simd_width]._elemwise_func_pow(mm, p)

    @staticmethod
    fn linspace(
        start: Scalar[dtype], stop: Scalar[dtype], size: Int
    ) -> MoVector[dtype, simd_width]:
    ...
    @staticmethod
    fn meshgrid(
        mv1: MoVector[dtype, simd_width], mv2: MoVector[dtype, simd_width]
    ) -> (MoMatrix[dtype, simd_width], MoMatrix[dtype, simd_width]):
    ...
    ](mm: MoMatrix[dtype, simd_width], s: Scalar[dtype]) -> MoMatrix[
        dtype, simd_width
    ]:
    ...
    fn _elemwise_matrix_matrix[
        ...
        mm: MoMatrix[dtype, simd_width], mat: MoMatrix[dtype, simd_width]
    ) -> MoMatrix[dtype, simd_width]:
        var new_mat = MoMatrix[dtype, simd_width](mm.rows, mm.cols)
    ...
    fn _elemwise_func_1[
        ...
        fn _fun[width: Int](iv: Int) -> None:
            new_vec._vec_ptr.store[width=width](
                iv, func(mv._vec_ptr.load[width=width](iv))
            )
    ...
    fn _elemwise_func_1[
        ...
        fn _fun[width: Int](iv: Int) -> None:
            new_mat._mat_ptr.store[width=width](
                iv, func(mm._mat_ptr.load[width=width](iv))
            )
    ...
    @staticmethod
    fn _elemwise_func_pow(
        mv: MoVector[dtype, simd_width], p: Scalar[dtype]
    ) -> MoVector[dtype, simd_width]:
        var new_vec = MoVector[dtype, simd_width](mv.size)
        ...
        fn _fun[width: Int](iv: Int) -> None:
            new_vec._vec_ptr.store[width=width](
                iv,
                pow(mv._vec_ptr.load[width=width](iv), SIMD[dtype, width](p)),
            )
    ...
    @staticmethod
    fn _elemwise_func_pow(
        mm: MoMatrix[dtype, simd_width], p: Scalar[dtype]
    ) -> MoMatrix[dtype, simd_width]:
        var new_mat = MoMatrix[dtype, simd_width](mm.rows, mm.cols)
        ...
        fn _fun[width: Int](iv: Int) -> None:
            new_mat._mat_ptr.store[width=width](
                iv,
                pow(mm._mat_ptr.load[width=width](iv), SIMD[dtype, width](p)),
            )
    ...
    @staticmethod
    fn _minmax(
        ptr: UnsafePointer[Scalar[dtype]], size: Int
    ) -> (Scalar[dtype], Scalar[dtype]):
