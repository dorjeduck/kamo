...
struct MoMatrix[dtype: DType, simd_width: Int](
    ...
    fn __init__(out self, rows: Int, cols: Int, val: Scalar[dtype]):
        self._mat_ptr = UnsafePointer[Scalar[dtype]].alloc(rows * cols)
    ...
    @always_inline
    fn __init__(out self, rows: Int, cols: Int, *data: Scalar[dtype]):
    ...
    @always_inline
    fn __init__(
        out self, rows: Int, cols: Int, owned list: List[Scalar[dtype]]
    ):
    ...
    @always_inline
    fn __init__(
        out self, rows: Int, cols: Int, mv: MoVector[dtype, simd_width]
    ):
        self.rows = rows
        self.cols = cols
        self._mat_ptr = UnsafePointer[Scalar[dtype]].alloc(
            self.rows * self.cols
        )
    ...
    @always_inline
    fn __init__(
        out self, rows: Int, cols: Int, owned ptr: UnsafePointer[Scalar[dtype]]
    ):
        self.rows = rows
        self.cols = cols
        self._mat_ptr = ptr
    ...
    fn __setitem__(self, elem: Int, val: Scalar[dtype]):
        self._mat_ptr.store[width=1](elem, val)

    fn __setitem__(self, row: Int, col: Int, val: SIMD[dtype, 1]):
    ...
    @always_inline
    fn insert(
        self, val: Scalar[dtype], start_pos: Int, num: Int, step: Int = 1
    ):
        ##todo optimize?
        # for i in range(num):
        ...
        fn _val[width: Int](iv: Int):
            self._mat_ptr.offset(start_pos + iv * step).strided_store[
                width=width
            ](val, step)
    ...
    @always_inline
    fn get_row(self, row: Int, mv: MoVector[dtype, simd_width]):
        # @parameter
    ...
    fn get_col(self, col: Int) -> MoVector[dtype, simd_width]:
        ...
        fn _get_col[width: Int](iv: Int):
            res._vec_ptr.store[width=width](
                iv,
                self._mat_ptr.offset(col + iv * self.cols).strided_load[
                    width=width
                ](self.cols),
            )
    ...
    fn to_numpy(self) raises -> PythonObject:
        ...
        memcpy(
            np_arr.__array_interface__["data"][0].unsafe_get_as_pointer[
                dtype
            ](),
            self._mat_ptr,
            self.rows * self.cols,
