...
struct GaussianRBF(EdgeFunc):
    ...
    fn basis_function(self, i: Int, x: MV) -> MV:
        ...
        for nn in range(x.size):
            res[nn] = MN.exp(-((x[nn] - center) * (x[nn] - center)) / (2 * self.sigma * self.sigma))
        return res
    ...
    fn basis_function_derivative(self, i: Int, x: MV) -> MV:
        ...
        var center = self.centers[i]
        for nn in range(x.size):
            var value = MN.exp(-((x[nn] - center) * (x[nn] - center)) / (2 * self.sigma * self.sigma))
    ...
    fn set_uniform_centers(inout self):
